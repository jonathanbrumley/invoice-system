...
import { SQLiteInvoiceStore } from "../SQLiteInvoiceStore";
import { Invoice, InvoiceStatus } from "@my/shared";
import * as fs from "fs";
import * as path from "path";

const testDatabasePath = path.join(__dirname, "test-sqlite-invoice-store.db");
...
beforeAll(async () => {
  ...
  store = await SQLiteInvoiceStore.initialize(testDatabasePath);
});
